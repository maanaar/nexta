  ...
  return (
      <div className="w-full flex flex-col gap-6 mt-10">
        ...
        <div className="w-full rounded-t-xl bg-[#D9D9D9]/95 min-h-screen py-10 flex flex-col items-center">
          ...
          <div className="w-[95%] px-4 sm:px-6 lg:px-12 flex flex-row gap-x-6 mt-6 items-start justify-center">
            ...
            {orderedStates.map((state) => {
              ...
              return (
                <div
                  key={state}
                  className="bg-gray-100 w-[45%] rounded-xl p-4 shadow-lg flex flex-col border border-gray-300/40"
                >
                  {/* Column Header */}
                  <h2 className="font-semibold text-center mb-4 rounded-xl py-2 shadow bg-gradient-to-r from-[#3485A1]/10 via-[#43739c]/25 to-[#3371A9]/10 text-gray-800 flex flex-row items-center justify-center gap-2">
                    <span>{state}</span>
                  ...
                  {/* Cards */}
                  <div className="flex flex-col rounded-2xl shadow-inner gap-3 py-2 pr-2 overflow-y-auto max-h-[70vh]">
                    {count > 0 ? (
                      grouped[state].map((p) => (
                        <div
                          key={p.id}
                          className="bg-white rounded-lg px-4 py-3 shadow hover:shadow-md transition-all text-sm border border-gray-200"
                        >
                          <div className="font-semibold text-gray-800">{p.patientName}</div>
    
                          <div onClick={() => router.push(`/patient/${p.id}`)} className="flex flex-col text-xs text-gray-600 mt-1">
